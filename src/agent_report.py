...
def build_prompt(summary: str, fecha_inicio: str, fecha_fin: str, evolucion: str) -> str:
...
EVOLUCIÓN SEMANAL DESDE EL LANZAMIENTO DEL BOT:
{evolucion}

Escribe un reporte narrativo en español, en prosa continua (sin títulos, sin secciones, sin bullets, sin markdown). Un solo bloque de texto de 5 a 7 párrafos cortos.
...
def main():
    ...
    # 2. Obtener evolución semanal desde lanzamiento
    evo_df = db.get_weekly_evolution(args.fecha_fin)
    evolucion_lines = [
    ...
    prompt    = build_prompt(summary, args.fecha_inicio, args.fecha_fin, evolucion)
