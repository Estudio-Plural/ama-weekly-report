...
# ── Evolución semanal desde lanzamiento ──────────────────────────────────────

def get_weekly_evolution(fecha_fin: str) -> pd.DataFrame:
    """Usuarios activos por semana desde el lanzamiento hasta fecha_fin."""
    sql = """
        SELECT
            DATE_TRUNC('week', created_at AT TIME ZONE 'America/Bogota')::date::text AS semana,
            COUNT(DISTINCT client_number) AS usuarios
        FROM ama.ama_session_start_table
        WHERE DATE(created_at AT TIME ZONE 'America/Bogota') BETWEEN :fecha_inicio AND :fecha_fin
          AND client_number ~ '^(57|59)'
        GROUP BY semana
        ORDER BY semana
    """
    return query_df(sql, {"fecha_inicio": START_DATE, "fecha_fin": fecha_fin})
